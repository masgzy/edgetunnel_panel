...
func main() {
	...
	runHTTP(handler, cfg, scMode, sup)
}
...
// runHTTP 启动 HTTP 监听，打印访问地址，并阻塞等待服务错误或退出信号。
// 退出时优雅关闭 HTTP 并回收 subconverter 子进程（监督者统一管理），避免孤儿进程。
func runHTTP(handler http.Handler, cfg *config.RuntimeConfig, scMode string, sup *scSupervisor) {
