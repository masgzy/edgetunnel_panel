...
const translations = {
  'zh': {
    ...
    // 完整配置（config.yml 全节设置）
    'full_config': '完整配置', 'full_config_subtitle': 'config.yml 全部设置项，保存后热重载',
    'fc_group_remote': '远程对接', 'fc_group_auth': '鉴权与口令', 'fc_group_nodes': '节点解析',
    'fc_group_flag': '国旗补全', 'fc_group_proxyip': 'ProxyIP 兑底',
    'fc_group_files': '文件路径（只读）', 'fc_group_app': '应用（只读）',
    'fc_control_domain': '控制端域名', 'fc_admin_url': '面板 config.json 地址（留空自动拼接）',
    'fc_request_timeout': '请求超时（秒）', 'fc_sub_port': '订阅端口', 'fc_uuid_ttl': 'UUID 缓存有效期（秒，0=默认 86400）',
    'fc_login_password': '远程控制端登录口令', 'fc_web_password': '本面板控制台口令',
    'fc_userinfo_expire': '订阅到期时间', 'fc_leave_empty': '留空保持不变',
    'fc_web_empty': '清空 web_password（关闭控制台鉴权）',
    'fc_bare_ip_role': '无 @ 行角色', 'fc_flag_iata': '三字码补旗（HKG→🇭🇰）', 'fc_flag_iso2': '二字码补旗（HK/US）',
    'fc_proxy_global': '全局 ProxyIP', 'fc_proxy_detect': '无码时经 cdn-cgi/trace 探测 cfcolo',
    'fc_by_region': '按区域码指定 ProxyIP', 'fc_add_region': '添加区域', 'fc_region_code': '三字码', 'fc_region_value': 'ProxyIP',
    'stat_item_usage': 'CF 今日请求', 'usage_unavailable': '暂无 CF 用量数据（面板未上报或尚未拉取）',
    'usage_tooltip': '今日 Pages + Workers 请求总数（来自远程面板）', 'usage_tooltip_pct': '今日请求量已用 {pct}%',
    'one_click_import': '一键导入', 't_import_need_vless': '该客户端仅接受 vless 原生订阅，请先把输出格式切到 vless',
    't_import_need_sub': '该客户端需要订阅格式（mihomo/clash/singbox 等），vless 直链不支持',
    'th_colo': '区域', 'probe_colo': '探测区域', 't_probe_running': '探测中…', 't_probe_fail': '探测请求失败',
    't_probe_none': '全部未命中（非 CF 出口或不可达）', 'colo_hint': '区域码来自 cdn-cgi/trace 的 cfcolo',
  ...
  'en': {
    ...
    'config_file': 'Config File', 'save_btn': 'Save', 'config_subtitle': 'config.yml', 'config_save_hint': 'Saved changes are validated and hot-reloaded; app.host/port/debug need a restart (or click "Restart Service")', 'copy_all': 'Copy All', 'restart_service': 'Restart Service', 'cd_restart_title': 'Restart Service', 'cd_restart_content': 'Restart edt_panel now? Subscriptions and panel will be briefly unavailable.', 't_restarting': 'Restarting, waiting for recovery…', 't_restart_fail': 'Restart failed', 't_config_hot_ok': 'Saved & hot-reloaded', 't_config_saved_partial': 'Saved. Restart required for: {items}', 't_config_invalid': 'Invalid config, not saved',
    ...
    // Full config (all config.yml sections)
    'full_config': 'Full Config', 'full_config_subtitle': 'All config.yml settings, hot-reloaded on save',
    'fc_group_remote': 'Remote', 'fc_group_auth': 'Auth', 'fc_group_nodes': 'Node Parsing',
    'fc_group_flag': 'Flag Emoji', 'fc_group_proxyip': 'ProxyIP Fallback',
    'fc_group_files': 'Files (read-only)', 'fc_group_app': 'App (read-only)',
    'fc_control_domain': 'Control Domain', 'fc_admin_url': 'Panel config.json URL (auto-join if empty)',
    'fc_request_timeout': 'Request Timeout (s)', 'fc_sub_port': 'Sub Port', 'fc_uuid_ttl': 'UUID Cache TTL (s, 0=default 86400)',
    'fc_login_password': 'Remote Login Password', 'fc_web_password': 'Console Password',
    'fc_userinfo_expire': 'Subscription Expire', 'fc_leave_empty': 'Leave empty to keep',
    'fc_web_empty': 'Clear web_password (disable console auth)',
    'fc_bare_ip_role': 'Bare IP Role', 'fc_flag_iata': 'IATA codes (HKG→🇭🇰)', 'fc_flag_iso2': 'ISO2 codes (HK/US)',
    'fc_proxy_global': 'Global ProxyIP', 'fc_proxy_detect': 'Detect cfcolo via cdn-cgi/trace',
    'fc_by_region': 'ProxyIP by Region Code', 'fc_add_region': 'Add Region', 'fc_region_code': 'Code', 'fc_region_value': 'ProxyIP',
    'stat_item_usage': 'CF Requests Today', 'usage_unavailable': 'No CF usage data (not reported or not fetched yet)',
    'usage_tooltip': 'Pages + Workers requests today (from remote panel)', 'usage_tooltip_pct': '{pct}% of daily quota used',
    'one_click_import': 'Import', 't_import_need_vless': 'This client only accepts native vless subs — switch output format to vless',
    't_import_need_sub': 'This client needs a sub format (mihomo/clash/singbox), not raw vless links',
    'th_colo': 'Colo', 'probe_colo': 'Probe Colo', 't_probe_running': 'Probing…', 't_probe_fail': 'Probe request failed',
    't_probe_none': 'None matched (non-CF egress or unreachable)', 'colo_hint': 'Colo comes from cdn-cgi/trace',
